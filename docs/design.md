...
└── Google フォト/
    ├── 集合写真/
    │   ├── DSC08207.jpg   (+ .supplemental-metadata.json)
    │   ├── DSC08211.jpg   (+ .supplemental-metadata.json)
    │   ├── DSC082710.jpg  (+ .supplemental-metadata.json)
    │   ├── DSC082713.jpg  (+ .supplemental-metadata.json)
    │   └── メタデータ.json          ← アルバムメタデータ（sidecar ではない）
    └── 第１９回図書館総合展/
        ├── IMG_6497.JPG … IMG_6517.JPG (17 件) + IMG_6498.PNG (1 件)
...
| 項目 | 値 |
| --- | --- |
| 総ファイル数 | 48（うち `.DS_Store` 2） |
| メディア | 22（JPG 17 / jpg 4 / PNG 1） |
| JSON | 24（メディア sidecar 22 + アルバムメタデータ 2） |
| 動画 | **0 件**（`.mp4` / `.mov` は本データに存在しない） |
| HEIC | **0 件** |
| アルバム | 2 |

### 3.3 JSON sidecar の命名形式

- 本データは **全 22 件が `<元のファイル名フルネーム>.supplemental-metadata.json` 形式**（例: `IMG_6497.JPG.supplemental-metadata.json`）。旧形式 `IMG_6497.JPG.json` は 0 件。
- 拡張子の大文字・小文字は元ファイルと完全一致（`.JPG` は `.JPG`、`.jpg` は `.jpg`、`.PNG` は `.PNG`）。
- **切り詰め（truncation）は本データには 0 件**。JSON ファイル名の最大長は 40 文字（`DSC082713.jpg.supplemental-metadata.json`）で、Google の切り詰め閾値（後述 51 文字）に達していない。
- **duplicate suffix `(1)` `(2)` も本データには 0 件。**
- JSON 内の `"title"` は **全 22 件でメディアの basename と完全一致**（`title == basename(media)`）。
- ファイル名・ディレクトリ名の Unicode 正規化: 本データの日本語名（`集合写真`、`第１９回図書館総合展`）は漢字・全角数字のみで構成され、NFC/NFD で同一バイト列になる（正規化非依存）。**したがって本データは NFD 問題を検出できない。**
...
```jsonc
// 集合写真/メタデータ.json
{ "title": "集合写真", "description": "", "access": "protected",
  "date": { "timestamp": "1774575640", "formatted": "2026/03/27 1:40:40 UTC" } }

// 第１９回図書館総合展/メタデータ.json
{ "title": "第１９回図書館総合展" }
```
...
### 3.5 `集合写真` — EXIF 撮影日時なしのケース

| ファイル | EXIF 日時 | XMP | JSON photoTakenTime (UTC) | JSON creationTime (UTC) |
| --- | --- | --- | --- | --- |
| DSC08207.jpg | **なし** | `XMP-xmp:MetadataDate = 2026:03:26 13:59:54+09:00` | 2026-03-25 12:59:54 | 2026-03-27 01:40:40 |
| DSC08211.jpg | **なし** | `XMP-xmp:MetadataDate = 2026:03:26 13:57:10+09:00` | 2026-03-25 12:57:10 | 2026-03-27 01:40:40 |
| DSC082710.jpg | **なし** | なし | 2026-03-25 15:16:42 | 2026-03-27 01:40:40 |
| DSC082713.jpg | **なし** | なし | 2026-03-25 15:17:34 | 2026-03-27 01:40:40 |
...
1. **`DateTimeOriginal` / `CreateDate` / `ModifyDate` がいずれも存在しない。** `Make` / `Model` もない。
   ...
   `DSC082710.jpg` / `DSC082713.jpg` に至っては **XMP も ICC も無く、日時系メタデータが完全にゼロ**。
2. **`XMP-xmp:MetadataDate` は撮影日時ではない。** DSC08207 では `photoTakenTime` と **ちょうど 16 時間** ずれている
   （`2026-03-26 04:59:54 UTC` vs `2026-03-25 12:59:54 UTC`）。DSC08211 も同じく 16 時間差。
   これはメタデータ書き込み日時（Adobe 書き出し時刻）であり、**撮影日時候補に採用してはならない**。
   → 「日時が存在するから使う」の典型的な罠。
3. **`creationTime` は 4 ファイルすべてで同一値**（`2026-03-27 01:40:40 UTC`）であり、
...
### 3.6 `第１９回図書館総合展` — EXIF 撮影日時ありのケース

代表例 `IMG_6497.JPG`（iPhone 6s / iOS 11.1）:

```
IFD0:ModifyDate            2017:11:07 08:40:23     ← ローカル時刻、タイムゾーン情報なし
ExifIFD:DateTimeOriginal   2017:11:07 08:40:23     ← 同上
ExifIFD:CreateDate         2017:11:07 08:40:23     ← 同上
ExifIFD:SubSecTimeOriginal 515
ExifIFD:OffsetTimeOriginal （存在しない）           ← iOS 11 は未記録
GPS:GPSDateTime            2017:11:06 23:40:13Z    ← UTC
JSON photoTakenTime        1510011623 = 2017-11-06 23:40:23 UTC
JSON creationTime          1510016412 = 2017-11-07 01:00:12 UTC
```

**全 17 枚の JPEG について、`DateTimeOriginal` を UTC とみなして `photoTakenTime` を引いた差分は
例外なくちょうど `+32400 秒 = +9 時間 = JST` だった。**
...
- `IMG_6498.PNG`（スクリーンショット）には **EXIF が一切ない**が、
  `XMP-photoshop:DateCreated = 2017:11:07 08:48:03` を持ち、これは `photoTakenTime`（`2017-11-06 23:48:03 UTC`）を
  **JST に変換した値と完全一致する**。
  → **`XMP-photoshop:DateCreated` は撮影日時として信頼できる**（`XMP:MetadataDate` とは正反対）。
  XMP のどのタグかによって意味が全く違うため、**タグ単位のホワイトリスト運用が必須**。
- `creationTime` はこのアルバムでも **18 件すべて同一値**（`2017-11-07 01:00:12 UTC`）。§3.5-3 と同じ結論。
...
| MP4 へ `-api QuickTimeUTC=1` 付きで `QuickTime:CreateDate=2017:11:07 08:40:23+09:00` | 成功。**ファイル内には UTC の `2017:11:06 23:40:23` が格納される**（＝ `photoTakenTime` そのもの） |
| 同じ MP4 を `-api QuickTimeUTC=1` **なし**で読むと | `2017:11:06 23:40:23` と表示される → **オプションを付け忘れると 9 時間ずれる典型事故** |
| MP4 へ `Keys:CreationDate` | `2017:11:07 08:40:23+09:00` としてオフセット付きで格納される（Apple 系がここを見る） |
...
python -m photo_date_restore "sources/Takeout/Google フォト/集合写真"
...
```jsonc
{
  "title": "IMG_6497.JPG",           // 元のファイル名。duplicate 時は (n) を含まないことがある（GPTH #59）
  "description": "",
  "imageViews": "33",
  "creationTime":  { "timestamp": "1510016412", "formatted": "..." },  // Google フォト登録日時（UTC）
  "photoTakenTime":{ "timestamp": "1510011623", "formatted": "..." },  // 撮影日時（UTC）★これが本命
  "geoData":     { "latitude": 35.4591944, "longitude": 139.6342917, "altitude": 23.21, ... },
...
- `geoData` が全て `0.0` のケースあり（`DSC08207.jpg`, `IMG_6498.PNG`）→ **`0.0/0.0` は「位置情報なし」として扱う**（Null Island 問題）
- `geoDataExif` は存在しないことがある
- `people` は存在しないことがある
- `timestamp` は **文字列**（`"1510011623"`）。数値ではない
...
- **`MATCH_TZ_*` の 4 分割が「単純な 15 分刻み・14 時間以内だけで一致断定しない」の実装。**
  ...
  status で区別できるようにする。
- 本データの `第１９回図書館総合展` JPEG 17 枚は `implied_offset = 32400`（= 900 × 36、誤差 0）で、
...
| 3 | **同一ディレクトリ内の兄弟ファイル**で、上記 1・2 いずれかにより確定したオフセットが
      **既定 3 件以上、矛盾なく一致**する場合、その値 | `INFERRED` | `--no-tz-from-siblings` で無効化可（既定 ON）。JSON `geoData` の緯度経度からの推定
      ...
      `IMG_6498.PNG`（EXIF なし）に対し、同一アルバムの JPEG 17 枚（tier GPS で確定済み）から +09:00 を導ける |
...
2. **Pillow は画像を再エンコードする**（`Image.open().save()`）。31MB の JPEG に対してこれを行うのは
   「原本を壊さない」原則の明確な違反。
3. **piexif は EXIF セグメントを丸ごと再構築する**ため、Apple MakerNote（本データの `IMG_6497.JPG` は
...
os.utime で mtime=2017-11-07 08:40:23 に設定
    →     mtime=2017-11-07 08:40:23  btime=2017-11-07 08:40:23   ← btime も追随して下がる

続けて os.utime で mtime=2030-01-01 に設定
    →     mtime=2030-01-01 00:00:00  btime=2017-11-07 08:40:23   ← btime は上がらない
```
...
```
INPUT/Google フォト/集合写真/
├── DSC08207.jpg                                     ← 修復済み（EXIF + mtime）
└── (DSC08207.jpg.supplemental-metadata.json は移動済み)
...
# 3. 特定アルバムだけ
photo-date-restore "sources/Takeout/Google フォト/集合写真" --output out --apply
...
| 2 | EXIF あり・JSON あり・Δ=32400・`OffsetTimeOriginal=+09:00` 明示 | `EXIF_JSON_MATCH_TZ_EXPLICIT`、offset `+09:00` |
...
| 3 | Δ=-32400（西側）・GPS 裏付けあり | `EXIF_JSON_MATCH_TZ_GPS`、offset `-09:00` |
...
`sources/Takeout` を入力として、以下を期待値として固定する（すべて §3 の実測に基づく）。

### 22.1 `第１９回図書館総合展`（EXIF あり）

| 検証項目 | 期待値 |
| --- | --- |
| 検出メディア数 | 18（JPEG 17 + PNG 1） |
| sidecar 対応付け | 18 件すべて T1（`title == basename`）、`json_candidates == 1` |
| `メタデータ.json` | sidecar として使われない（`ALBUM_METADATA` として記録） |
| JPEG 17 件の status | すべて `EXIF_JSON_MATCH_TZ_GPS`（**追補**。`OffsetTimeOriginal` は記録が無いため `_EXPLICIT` にはならない） |
| JPEG 17 件の `difference_seconds` | すべて `32400` |
| JPEG 17 件の `implied_offset_seconds` | すべて `32400` |
| JPEG 17 件の `selected_datetime_source` | `EXIF_DATETIME_ORIGINAL` |
| JPEG 17 件の `timezone_source` | `GPS`（`GPSDateTime` から +09:00 を導出） |
| JPEG 17 件の EXIF 書き込み | **行われない**（既に正しいため） |
| `IMG_6497.JPG` の `selected_datetime` | `2017-11-07T08:40:23+09:00` |
| `IMG_6497.JPG` の `new_mtime` | `2017-11-07T08:40:23+09:00` |
| `IMG_6498.PNG` の status | `EXIF_JSON_MATCH_TZ_INFERRED`（`XMP-photoshop:DateCreated` を §6.1 ホワイトリストの既存日時候補として採用するため、ケース【A】扱いになる。EXIF 相当のタグが無いための `JSON_TIME_USED` にはならない） |
| `IMG_6498.PNG` の `selected_datetime` | `2017-11-07T08:48:03+09:00`（既存の `XMP-photoshop:DateCreated` をそのまま採用。書き換えない） |
| `IMG_6498.PNG` の `timezone_source` | `INFERRED`（同一アルバムの JPEG 17 件が GPS 根拠で確認済みの +09:00 を、この PNG 自身の GPS 欠如を補って裏付ける。`--timezone` 指定時は `CLI`） |
| `creationTime` の使用 | **0 件**（全 18 件で同一値 `2017-11-07T01:00:12Z` であることも記録。v1.0 では `selected_datetime` の算出に使わない） |
...
| --- | --- |
| 検出メディア数 | 4 |
| sidecar 対応付け | 4 件すべて T1 |
| **`--timezone` 未指定の場合**（既定・追補） | 4 件すべて `JSON_TIME_MTIME_ONLY`。`timezone_source = NONE`、`message` に `TIMEZONE_REQUIRED_FOR_METADATA`。EXIF は一切書かず、mtime のみ JSON `photoTakenTime`（UTC）から復元する。**根拠**: 4 ファイルとも EXIF/GPS 情報が皆無で、同一ディレクトリ内でオフセットを確認できるファイルも無いため、§8.2 のラダーが tier 5（`NONE`）まで落ちる |
| **`--timezone Asia/Tokyo` 指定時** | 4 件すべて `JSON_TIME_USED`。`timezone_source = CLI`、`selected_datetime_source = PHOTO_TAKEN_TIME` |
| `DSC08207.jpg` の `selected_datetime`（`--timezone Asia/Tokyo` 時） | `2026-03-25T21:59:54+09:00` |
| `DSC08211.jpg` | `2026-03-25T21:57:10+09:00` |
| `DSC082710.jpg` | `2026-03-26T00:16:42+09:00` |
| `DSC082713.jpg` | `2026-03-26T00:17:34+09:00` |
| `XMP:MetadataDate` の扱い | **採用されない**。`message` に「MetadataDate は撮影日時ではないため除外」を記録 |
| 書き込みタグ（`--timezone` 指定時のみ） | `EXIF:DateTimeOriginal` / `EXIF:CreateDate` / `EXIF:OffsetTimeOriginal=+09:00` / `OffsetTimeDigitized` |
| 既存 XMP・ICC・Adobe APP14 | **保持されること**（書き込み後に `exiftool -XMP:all` で確認） |
| `creationTime` の使用 | **0 件**（v1.0 では読み取り専用） |
...
| --- | --- |
| `AMBIGUOUS_JSON` | 0 件 |
| `EXIF_JSON_CONFLICT` | 0 件 |
| `NO_DATE` | 0 件 |
| `ERROR` | 0 件 |
| `.DS_Store` | 走査対象外（レポートに現れない） |
| copy mode 出力 | `out/Google フォト/{集合写真,第１９回図書館総合展}/` にメディア 22 件のみ。JSON 0 件 |
| **2 回目実行** | 全 22 件が `NO_CHANGE`。出力ファイルの sha256 が 1 回目と完全一致 |
| **`sources/` の不変性** | 実行前後で全ファイルの sha256 / サイズ / mtime_ns が一致し、ファイル数も増減しない |
| 読み戻し検証 | 全 22 件で成功。`VERIFY_FAILED` 0 件 |

### 22.4 in-place の受入テスト

`sources/` の **コピー**（tmpdir）に対して実施する。

| 検証項目 | 期待値 |
| --- | --- |
| `--in-place --apply` 実行後 | メディア 22 件の EXIF / mtime が §22.1〜22.2 どおり |
| `_original` ファイル | 実行後に 1 件も残っていない |
| JSON | 移動していない（`--move-json` 未指定のため） |
| `--move-json BACKUP` 併用時 | メディア側に JSON が 0 件。`BACKUP/Google フォト/<アルバム>/*.json` が 22 件 |
| `メタデータ.json` 2 件 | **移動されずに元の場所に残る** |
| `BACKUP/_sidecar_index.csv` | 22 行 + ヘッダ |
| 退避後の再実行 | sidecar が元位置にないため全件 `NO_JSON`。本体への再書き込みなし |
...
## 付録 B. 実データ数値サマリ（テスト期待値の根拠）

```
第１９回図書館総合展 (18 media)
  IMG_6497.JPG  DTO 2017:11:07 08:40:23  GPS 2017:11:06 23:40:13Z  ptt 2017-11-06 23:40:23Z  Δ=+32400
  IMG_6498.PNG  DTO なし (XMP DateCreated 2017:11:07 08:48:03)     ptt 2017-11-06 23:48:03Z  Δ=—
  IMG_6500.JPG  DTO 2017:11:07 09:03:35                            ptt 2017-11-07 00:03:35Z  Δ=+32400
  IMG_6503.JPG  DTO 2017:11:07 09:06:35                            ptt 2017-11-07 00:06:35Z  Δ=+32400
  IMG_6504.JPG  DTO 2017:11:07 09:07:01                            ptt 2017-11-07 00:07:01Z  Δ=+32400
  IMG_6505.JPG  DTO 2017:11:07 09:09:59                            ptt 2017-11-07 00:09:59Z  Δ=+32400
  IMG_6506.JPG  DTO 2017:11:07 09:10:05                            ptt 2017-11-07 00:10:05Z  Δ=+32400
  IMG_6507.JPG  DTO 2017:11:07 09:15:41                            ptt 2017-11-07 00:15:41Z  Δ=+32400
  IMG_6508.JPG  DTO 2017:11:07 09:15:43                            ptt 2017-11-07 00:15:43Z  Δ=+32400
  IMG_6509.JPG  DTO 2017:11:07 09:25:24                            ptt 2017-11-07 00:25:24Z  Δ=+32400
  IMG_6510.JPG  DTO 2017:11:07 09:43:08                            ptt 2017-11-07 00:43:08Z  Δ=+32400
  IMG_6511.JPG  DTO 2017:11:07 09:45:37                            ptt 2017-11-07 00:45:37Z  Δ=+32400
  IMG_6512.JPG  DTO 2017:11:07 09:45:57                            ptt 2017-11-07 00:45:57Z  Δ=+32400
  IMG_6513.JPG  DTO 2017:11:07 09:46:10                            ptt 2017-11-07 00:46:10Z  Δ=+32400
  IMG_6514.JPG  DTO 2017:11:07 09:46:35                            ptt 2017-11-07 00:46:35Z  Δ=+32400
  IMG_6515.JPG  DTO 2017:11:07 09:46:36                            ptt 2017-11-07 00:46:36Z  Δ=+32400
  IMG_6516.JPG  DTO 2017:11:07 09:46:37                            ptt 2017-11-07 00:46:37Z  Δ=+32400
  IMG_6517.JPG  DTO 2017:11:07 09:46:37                            ptt 2017-11-07 00:46:37Z  Δ=+32400
  creationTime : 全 18 件で 2017-11-07 01:00:12Z（同一）
  カメラ       : Apple iPhone 6s / iOS 11.1、OffsetTimeOriginal なし、GPSDateTime あり（PNG 除く）

集合写真 (4 media)
  DSC08207.jpg   EXIF 日時なし / XMP MetadataDate 2026:03:26 13:59:54+09:00  ptt 2026-03-25 12:59:54Z
  DSC08211.jpg   EXIF 日時なし / XMP MetadataDate 2026:03:26 13:57:10+09:00  ptt 2026-03-25 12:57:10Z
  DSC082710.jpg  メタデータ皆無                                              ptt 2026-03-25 15:16:42Z
  DSC082713.jpg  メタデータ皆無                                              ptt 2026-03-25 15:17:34Z
  creationTime : 全 4 件で 2026-03-27 01:40:40Z（同一、アルバム date と一致）
  MetadataDate と photoTakenTime の差 : ちょうど 16 時間（= 撮影日時ではない証拠）

全 22 件共通
  FileModifyDate : 2026-08-26 20:05〜20:06（= Takeout 解凍日時。撮影日時として無意味）
  JSON title     : 22/22 件で basename(media) と完全一致
  sidecar 形式   : 22/22 件が <basename>.supplemental-metadata.json（切り詰めなし・duplicate なし）
```
